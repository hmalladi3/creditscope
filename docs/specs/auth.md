...
- [x] **AUTH-BE-010**: The system shall require the `ADMIN` role for `POST`, `PUT`, and `DELETE` requests under `/api/companies/**`. *(The security rule covers all three methods; only the `POST` path currently has a controller behind it to exercise end-to-end — see API-BE-011.)*
